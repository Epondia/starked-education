...
const ContentUploader: React.FC<ContentUploaderProps> = ({
  ...
  const addFiles = async (newFiles: FileList) => {
    ...
    for (
      let i = 0;
      i < newFiles.length && validFiles.length < maxFiles - files.length;
      i++
    ) {
  ...
  const getOverallProgress = (): number => {
    ...
    const totalProgress = uploadingFiles.reduce(
      (sum, f) => sum + (f.progress?.progress || 0),
      0
    );
    ...
    <div className={`content-uploader ${className}`}>
      ...
      >
        ...
        <p className="text-sm text-gray-500 mb-4">
          Maximum {maxFiles} files, up to {Math.round(maxSize / 1024 / 1024)}MB
          each
        </p>
                    ...
                    {fileItem.status === 'uploading' && fileItem.progress && (
                      <div className="mt-2">
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                          {formatFileSize(fileItem.progress.bytesLoaded)} /{' '}
                          {formatFileSize(fileItem.progress.bytesTotal)}
                        </p>
                      </div>
                    )}
                    ...
                    {fileItem.status === 'completed' && fileItem.result && (
                      <div className="mt-2 text-xs text-gray-600">
                        ...
                        <p>
                          Gateway:{' '}
                          <a
                            href={fileItem.result.gatewayUrl}
                            target="_blank"
