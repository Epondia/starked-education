#![no_std]
use soroban_sdk::{contract, contractimpl, contracttype, Address, Env, String, Bytes, Symbol};
...
// Re-export upgrade functionality
pub use upgrade::*;
...
impl StarkEdContract {
    ...
    pub fn issue_credential(
        ...
        let count: u64 = env.storage().instance().get(&DataKey::CredentialCount).unwrap_or(0);
    ...
    pub fn get_credential(env: Env, credential_id: u64) -> Credential {
        env.storage().instance().get(&DataKey::Credential(credential_id)).unwrap_or_else(|| panic!("Credential not found"))
    }
    ...
    pub fn create_course(
        ...
        let count: u64 = env.storage().instance().get(&DataKey::CourseCount).unwrap_or(0);
        let course_id = count + 1;
        let course = Course { id: course_id, title, description, price };
    ...
    pub fn get_course(env: Env, course_id: u64) -> Course {
        env.storage().instance().get(&DataKey::Course(course_id)).unwrap_or_else(|| panic!("Course not found"))
    }

    pub fn get_credential_count(env: Env) -> u64 {
        env.storage().instance().get(&DataKey::CredentialCount).unwrap_or(0)
    }
...
// Upgrade functionality moved to upgrade module
pub use upgrade::*;
